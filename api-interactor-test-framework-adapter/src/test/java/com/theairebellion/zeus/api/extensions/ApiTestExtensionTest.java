...
class ApiTestExtensionTest {
    ...
    void testBeforeTestExecution_NoAnnotation() throws Exception {
        ...
        assertTrue(consumerList.isEmpty(), "No annotation => no consumer added => list is empty");
    }
    ...
    void testBeforeTestExecution_ShouldInitializeStoreWhenEmpty() throws Exception {
        ...
        assertAll(
                ...
                () -> assertEquals(1, actualConsumers[0].size(), "Consumer should be added to the actual list used by the extension")
        );
    }
    ...
    void testPostQuestCreation_ShouldModifyQuestStorageAndAuthenticate() throws Exception {
        ...
        var authAs = this.getClass().getDeclaredMethod("dummyTestMethod").getAnnotation(AuthenticateViaApiAs.class);

        var extension = new ApiTestExtension();
        var method = ApiTestExtension.class.getDeclaredMethod("postQuestCreation", String.class, String.class, AuthenticateViaApiAs.class);
        method.setAccessible(true);

        @SuppressWarnings("unchecked")
        var consumer = (Consumer<Quest>) method.invoke(extension, username, password, authAs);
