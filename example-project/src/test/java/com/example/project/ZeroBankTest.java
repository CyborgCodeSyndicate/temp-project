...
public class ZeroBankTest extends BaseTest {
    ...
    public void alertTest(Quest quest) {
        quest
                ...
                .select().selectItem(SelectFields.TF_FROM_ACCOUNT_DDL, "Credit Card(Avail. balance = $ -265)")
                .select().selectItem(SelectFields.TF_TO_ACCOUNT_DDL, "Loan(Avail. balance = $ 780)")
    ...
    public void labelTextValueTest(Quest quest) {
        quest
                ...
                .select().selectItem(SelectFields.PC_CURRENCY_DDL, "Mexico (peso)")
...
//                .text().getValue(TextFields.CONVERSION_AMOUNT_VALUE)
    ...
    public void paragraphTextValueTest(Quest quest) {
        quest
                ...
                .select().selectItem(SelectFields.SP_PAYEE_DDL, "Sprint")
                .link().click(LinkFields.SP_PAYEE_DETAILS_LINK)
//                .text().getValue("For 12119415161214 Sprint account")
                .select().selectItem(SelectFields.SP_PAYEE_DDL, "Bank of America")
                .link().click(LinkFields.SP_PAYEE_DETAILS_LINK)
//                .text().getValue("For 47844181491040 Bank of America account")
                .select().selectItem(SelectFields.SP_PAYEE_DDL, "Apple")
                .link().click(LinkFields.SP_PAYEE_DETAILS_LINK)
//                .text().getValue("For 48944145651315 Apple account")
                .select().selectItem(SelectFields.SP_PAYEE_DDL, "Wells Fargo")
                .link().click(LinkFields.SP_PAYEE_DETAILS_LINK)
//                .text().getValue("For 98480498848942 Wells Fargo account")
                .select().selectItem(SelectFields.SP_ACCOUNT_DDL, "Checking")
    ...
    public void tableAssert(Quest quest) {
        quest
                ...
                .select().selectItem(SelectFields.AA_TYPE_DDL, "Deposit")
