 ...
 *
 * <p>This class provides methods for extracting table row values, table elements, and performing
...
public class UiTableValidatorImpl implements UiTableValidator {
    ...
    *
    * <p>Extracts data based on the assertion target (row values, row elements, table values, or table elements),
   ...
   public <T> List<AssertionResult<T>> validateTable(final Object object, final Assertion... assertions) {
      ...
      for (Assertion assertion : assertions) {
         ...
         switch ((UiTablesAssertionTarget) assertion.getTarget()) {
            ...
            case TABLE_ELEMENTS -> {
               List<?> rows = (List<?>) object;
               List<List<SmartWebElement>> rowList =
                     rows.stream().map(TableReflectionUtil::extractElementsFromRow).toList();
    ...
    *
    * <p>This method provides an Allure step annotation for better test reporting
    ...
    *
    * <p>This method marks the assertion processing step in Allure reports
