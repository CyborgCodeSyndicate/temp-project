...
public class ShadowDomUtils {
    ...
    /**
     * Default wait and poll (in ms) if not specified.
     * You can tweak these or make them configurable.
     */
    private static final long DEFAULT_POLL_INTERVAL_MS = 500;


    /**
     * Converts Selenium {@code By} into a map {type, value}
     * used by our JS scripts.
     */
    ...
    /**
     * Returns the *first* matching element in the entire document (or within shadow roots),
    ...
    /**
     * Finds the *first* matching element across the entire document + any shadow roots.
     * Waits up to `DEFAULT_MAX_WAIT_MS`, polling every `DEFAULT_POLL_INTERVAL_MS`.
     */
    ...
    /**
     * Same as above, but starts from a given root WebElement.
    ...
    /**
     * JavaScript to return *all matching elements* (an array) in the entire document and shadow roots,
    ...
    /**
     * Finds *all* matching elements across the entire document + nested shadow roots.
     * Waits up to `DEFAULT_MAX_WAIT_MS` (polling every `DEFAULT_POLL_INTERVAL_MS`) until
     * at least one is found, then returns whatever is found at that time.
     */
    ...
    /**
     * JavaScript to return *all matching elements* starting from a given root element (including
    ...
    /**
     * Finds *all* matching elements in nested Shadow DOMs, starting from a given root element.
     * Waits up to `DEFAULT_MAX_WAIT_MS`, polling every `DEFAULT_POLL_INTERVAL_MS`.
     */
    ...
    /**
     * Checks if there is at least one element that has a Shadow Root
     * starting from a given WebElement (including that element itself).
     *
     * @param root The WebElement root
     * @return true if the element itself or any descendant has a shadow root
     */
