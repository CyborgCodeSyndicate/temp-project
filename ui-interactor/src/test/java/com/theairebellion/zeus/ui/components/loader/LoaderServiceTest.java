...
class LoaderServiceTest extends BaseUnitUITest {
   ...
   void testGetDefaultTypeShouldReturnNullWhenExceptionIsThrown() throws Exception {
      ...
      when(mockConfig.loaderDefaultType()).thenReturn("SomeType");
      ...
      try (
         ...
         reflectionMock.when(() -> ReflectionUtil.findEnumImplementationsOfInterface(
               LoaderComponentType.class,
               "SomeType",
               "com.example"
         )).thenThrow(new RuntimeException("Simulated failure"));

         // Use reflection to access the private static method
         Method method = LoaderService.class.getDeclaredMethod("getDefaultType");
         method.setAccessible(true);

         LoaderComponentType result = (LoaderComponentType) method.invoke(null);
