...
class AssertionTypesTest {
    ...
    @Test
    @DisplayName("Type compatibility should be verified")
    void testTypeCompatibilityCheck() {
        // Examples of compatible types
        assertAll(
                // Object types accept anything
                () -> assertTrue(isCompatible(AssertionTypes.IS, "string")),
                () -> assertTrue(isCompatible(AssertionTypes.IS, 42)),
                () -> assertTrue(isCompatible(AssertionTypes.NOT_NULL, new Object())),
                () -> assertTrue(isCompatible(AssertionTypes.IS_NULL, null)),

                // String types accept strings only
                () -> assertTrue(isCompatible(AssertionTypes.CONTAINS, "substring")),
                () -> assertTrue(isCompatible(AssertionTypes.STARTS_WITH, "")),
                () -> assertTrue(isCompatible(AssertionTypes.EQUALS_IGNORE_CASE, "test")),

                // Number types accept numeric values
                () -> assertTrue(isCompatible(AssertionTypes.GREATER_THAN, 10)),
                () -> assertTrue(isCompatible(AssertionTypes.GREATER_THAN, 10.5)),
                () -> assertTrue(isCompatible(AssertionTypes.LESS_THAN, 0)),

                // Collection types accept collections
                () -> assertTrue(isCompatible(AssertionTypes.EMPTY, List.of())),
                () -> assertTrue(isCompatible(AssertionTypes.NOT_EMPTY, List.of(1, 2, 3)))
        );

        // Examples of incompatible types
        assertAll(
                // String methods with non-strings
                () -> assertFalse(isCompatible(AssertionTypes.CONTAINS, 42)),
                () -> assertFalse(isCompatible(AssertionTypes.MATCHES_REGEX, List.of())),

                // Number methods with non-numbers
                () -> assertFalse(isCompatible(AssertionTypes.GREATER_THAN, "string")),
                () -> assertFalse(isCompatible(AssertionTypes.BETWEEN, new Object())),

                // Collection methods with non-collections
                () -> assertFalse(isCompatible(AssertionTypes.EMPTY, "string")),
                () -> assertFalse(isCompatible(AssertionTypes.ALL_NOT_NULL, 42)),
                () -> assertFalse(isCompatible(AssertionTypes.CONTAINS_ALL, new Object()))
        );
    }

    // Helper method to check if value type is compatible with assertion type
    private boolean isCompatible(AssertionTypes type, Object value) {
        if (value == null) {
            return true; // Null is acceptable for IS_NULL assertions
        }

        Class<?> supportedType = type.getSupportedType();
        return supportedType.isInstance(value);
    }
