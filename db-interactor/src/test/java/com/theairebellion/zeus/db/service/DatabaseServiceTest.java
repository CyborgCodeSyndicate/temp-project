...
class DatabaseServiceTest {
    ...
    void testQuery_ShouldReturnQueryResponse() {
        ...
        QueryResponse response = new QueryResponse(List.of(Map.of("id", 1)));

        when(query.query()).thenReturn("SELECT * FROM users");
    ...
    void testQueryWithJsonPath_ShouldReturnExtractedValue() {
        DbQuery query = mock(DbQuery.class);
        QueryResponse response = new QueryResponse(List.of(Map.of("name", "John Doe")));
        ...
        when(dbClientManager.getClient(any())).thenReturn(dbClient);
        when(dbClient.executeQuery(any())).thenReturn(response);
        when(jsonPathExtractor.extract(response.getRows(), jsonPath, String.class)).thenReturn(expectedValue);

        String result = databaseService.query(query, jsonPath, String.class);

        assertEquals(expectedValue, result);
        verify(jsonPathExtractor).extract(response.getRows(), jsonPath, String.class);
    }

    @Test
    void testValidate_ShouldReturnAssertionResults() {
        QueryResponse queryResponse = new QueryResponse(List.of(Map.of("id", 1)));
        Assertion<?> assertion = mock(Assertion.class);
        List<AssertionResult<Object>> expectedResults = List.of(mock(AssertionResult.class));

        when(queryResponseValidator.validateQueryResponse(queryResponse, assertion)).thenReturn((List) expectedResults);
    ...
    void testQueryAndValidate_ShouldReturnAssertionResults() {
        ...
        when(dbClientManager.getClient(any())).thenReturn(dbClient);
        when(dbClient.executeQuery(any())).thenReturn(response);
        when(queryResponseValidator.validateQueryResponse(response, assertion)).thenReturn((List) expectedResults);
